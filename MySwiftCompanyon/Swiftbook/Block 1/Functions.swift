...
// MARK: - опереатор defer

// Код в теле дефера выполняется только после выхода из области видимости где он был использован, например после выхода из функции
// Если в одной области два и более дефера - то они выполняются в обратном порядке
...
// Выбор большего или меньшего из переданых параметров
// value = max(x: Comparable,y: Comparable) -  или  min(...)
